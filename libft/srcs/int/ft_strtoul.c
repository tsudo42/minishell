...
/* ************************************************************************** */
static int	strtoul_read_base(const char **str_ptr, int base, int *status)
{
	const char	*str;
...
/* ************************************************************************** */
/*  If (prev_val * base + next_d > ULONG_MAX), returns -1.                    */
/* ************************************************************************** */
static int	strtoul_check_overflow( \
	unsigned long prev_val, unsigned int next_d, int base)
{
	unsigned long	max_div;
	unsigned long	max_mod;

	max_div = ULONG_MAX / base;
	max_mod = ULONG_MAX % base;
	if (prev_val > max_div)
		return (-1);
...
/*  Status: 0 means read nothing, 1 means successfully reading, -1 means OVF  */
/* ************************************************************************** */
static unsigned long	strtoul_read_value( \
	const char **str, int base, int *status)
{
	unsigned long	val;
	unsigned int	d;
	...
	while (**str != '\0')
	{
		...
		if ((int)d >= base)
			break ;
		if (*status != -1)
			*status = strtoul_check_overflow(val, d, base);
		if (*status != -1)
			val = val * base + d;
		(*str)++;
	}
...
/* ************************************************************************** */
static unsigned long	ft_strtoul_helper( \
	const char *str, char **endptr, int base)
{
	...
	if (*str == '+' || *str == '-')
		str++;
	base = strtoul_read_base(&str, base, &status);
	val = strtoul_read_value(&str, base, &status);
	if (status != 0)
		*endptr = (char *)str;
	if (status == -1)
	{
...
/* ************************************************************************** */
/*  This function mimics the original strtoul (c.f. man 3 strtoul).           */
...
unsigned long	ft_strtoul(const char *str, char **endptr, int base)
{
	...
	endptr_tmp = (char *)str;
	val = ft_strtoul_helper(str, &endptr_tmp, base);
	if (endptr_tmp == str)
		errno = EINVAL;
	if (endptr != NULL)
		*endptr = endptr_tmp;
